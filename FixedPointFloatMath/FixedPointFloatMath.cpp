...
FixedPointErrorCode parseFixedPointValue(const char *string, int size,
                                         FixedPointValue *outputValue) {
...
int compareEqualFixedPointValuesData(FixedPointValueData *first,
                                     FixedPointValueData *second) {
  return first->parts[0] == second->parts[0] &&
         first->parts[1] == second->parts[1] &&
         first->parts[2] == second->parts[2] &&
         first->parts[3] == second->parts[3];
}

int compareGreaterFixedPointValuesData(FixedPointValueData *first,
                                       FixedPointValueData *second) {
...
int compareLesserFixedPointValuesData(FixedPointValueData *first,
                                      FixedPointValueData *second) {
  return !compareEqualFixedPointValuesData(first, second) &&
         !compareGreaterFixedPointValuesData(first, second);
}

int compareGreaterFixedPointValues(FixedPointValue *first,
                                   FixedPointValue *second) {
  if (first->sign != second->sign) {
    return first->sign > second->sign;
  }

  return compareGreaterFixedPointValuesData(&first->value, &second->value) ^
         (first->sign == -1);
}

int compareEqualFixedPointValues(FixedPointValue *first,
                                 FixedPointValue *second) {
  if (first->sign != second->sign) {
    return 0;
  }
...
int compareGreaterEqualFixedPointValues(FixedPointValue *first,
                                        FixedPointValue *second) {
  if (first->sign != second->sign) {
    return first->sign > second->sign;
  }
  ...
  return compareEqualFixedPointValuesData(&first->value, &second->value) ||
         compareGreaterFixedPointValuesData(&first->value, &second->value) ^
             (first->sign == -1);
}

FixedPointErrorCode
shiftOneLeftFixedPointValuesData(FixedPointValueData *value,
                                 FixedPointValueData *output) {
...
FixedPointErrorCode
shiftOneRightFixedPointValuesData(FixedPointValueData *value,
                                  FixedPointValueData *output) {
  for (int i = 0; i < 16; i++) {
...
FixedPointErrorCode shiftLeftFixedPointValuesData(FixedPointValueData *value,
                                                  FixedPointValueData *output,
                                                  int shift) {
  assert(shift > 0);
  assert(shift < 16);
...
FixedPointErrorCode shiftRightFixedPointValuesData(FixedPointValueData *value,
                                                   FixedPointValueData *output,
                                                   int shift) {
  assert(shift > 0);
  assert(shift < 16);
...
FixedPointErrorCode addFixedPointValuesData(FixedPointValueData *first,
                                            FixedPointValueData *second,
                                            FixedPointValueData *outputValue) {
...
FixedPointErrorCode
substractFixedPointValuesData(FixedPointValueData *first,
                              FixedPointValueData *second,
                              FixedPointValueData *outputValue) {
...
FixedPointErrorCode addFixedPointValues(FixedPointValue *first,
                                        FixedPointValue *second,
                                        FixedPointValue *outputValue);
FixedPointErrorCode substractFixedPointValues(FixedPointValue *first,
                                              FixedPointValue *second,
                                              FixedPointValue *outputValue);
FixedPointErrorCode multiplyFixedPointValues(FixedPointValue *first,
                                             FixedPointValue *second,
                                             FixedPointValue *outputValue);
FixedPointErrorCode divideFixedPointValues(FixedPointValue *first,
                                           FixedPointValue *second,
                                           FixedPointValue *outputValue);
...
FixedPointErrorCode addFixedPointValues(FixedPointValue *first,
  ...
  if (first->sign == second->sign) {
    ...
    result = addFixedPointValuesData(&first->value, &second->value,
                                     &outputValue->value);
  }
  else {
    int isFirstGreaterThanSecond =
        compareGreaterFixedPointValuesData(&first->value, &second->value);

    if (first->sign == 1) {
      if (isFirstGreaterThanSecond) {
        outputValue->sign = 1;
        substractFixedPointValuesData(&first->value, &second->value,
                                      &outputValue->value);
      }
      else {
        outputValue->sign = -1;
        substractFixedPointValuesData(&second->value, &first->value,
                                      &outputValue->value);
      }
    }
    else {
...
FixedPointErrorCode
multiplyFixedPointValuesData(FixedPointValueData *first,
                             FixedPointValueData *second,
                             FixedPointValueData *outputValue) {
  ...
  for (int secondByteIndex = 0; secondByteIndex < 16; secondByteIndex++) {
    for (int firstByteIndex = 0; firstByteIndex < 16; firstByteIndex++) {
      unsigned int result =
          first->bytes[firstByteIndex] * second->bytes[secondByteIndex];
...
FixedPointErrorCode multiplyFixedPointValues(FixedPointValue *first,
  ...
  return multiplyFixedPointValuesData(&first->value, &second->value,
                                      &outputValue->value);
}

// return 0..16
int findClosestFixedPointValue(FixedPointValueData *target,
                               FixedPointValueData *value) {
...
FixedPointErrorCode divideFixedPointValues(FixedPointValue *first,
  ...
  for (int i = 15; i >= 0; i--) {
    if (firstCopyData.bytes[i] == 0) {
      ...
      (firstCopyData.bytes[i] < (1 << 4)) ? firstLeftByteIndex -= 1
                                          : firstLeftByteIndex;
      break;
    }
  }
  ...
  for (int i = 15; i >= 0; i--) {
    if (secondCopyData.bytes[i] == 0) {
      ...
      (secondCopyData.bytes[i] < (1 << 4)) ? secondLeftByteIndex -= 1
                                           : secondLeftByteIndex;
      break;
    }
  }
  ...
  while (true) {
    if (compareEqualFixedPointValuesData(&modValue, &divisor)) {
      outputValue->value.bytes[outputByteIndex / 2] +=
          1 * (outputByteIndex & 1 ? (16) : 1);
      return FixedPointErrorCode::NO_ERROR;
    }
    ...
    multiplyFixedPointValuesData(&divisor, &multipliyerFixedValue,
                                 &multipliyedDivisor);

    outputValue->value.bytes[outputByteIndex / 2] +=
        multipliyer * (outputByteIndex & 1 ? (16) : 1);
...
const char *evaluateExpression(const char *expression,
  ...
  FixedPointValue *fixedPointValues =
      (FixedPointValue *)malloc(countOfNumbers * sizeof(FixedPointValue));
  ...
  while (true) {
    if (*iterator == '\0') {
      error = parseFixedPointValue(currentFixedPointValueIterator,
                                   (iterator - currentFixedPointValueIterator),
                                   &(fixedPointValues[currentFloat]));
      if (isNextNegative) {
  ...
  for (int i = leftIndex; i < 4; i++) {
    ...
    resultString[symbolIndex] = (i == leftIndex && lowerValue == 0)
                                    ? '\0'
                                    : getHexDigitSymbol(lowerValue);
